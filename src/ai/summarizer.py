...
def chunk(start_index=0, text_messages: List = [], n=500):
    word_count = 0
    end_index = start_index
    for end_index, msg in enumerate(text_messages[start_index:]):
        word_count += len(msg.split())
        if word_count > n:
            word_count = 0
            break
    
    return end_index, "".join(text_messages[start_index:end_index])


summarizer_prompt = PromptTemplate.from_template(
    """Summarize the given developers' comments on {repo_info}, preserving the meaning and details into around 150-200 around.
    Commments:
    {input}"""
)
...
@chain
def summarizer(llm):
    summ = summarizer_prompt | llm | StrOutputParser()


if __name__ == "__main__":
    # sample_data = functions.get_issue_comment("internetarchive","openlibrary",8623)
    sample_data = [
        "can i work on this?\r\n",
    ...
    a, b = chunk(0, sample_data)
    print(a)
    c, d = chunk(start_index=a-8, text_messages=sample_data)
    print(len(sample_data), c)
    print(d)
